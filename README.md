...
Dataset
Source: Breast Cancer Gene Expression Profiles (METABRIC) — Kaggle
...
8. Per-class + cross-model biomarker ranking	`wgcna_after_ml.py`	`mean(
9. Directional check & stability check on top candidates	`wgcna_after_ml.py`	`shap.summary_plot()`, repeated train-test splits
Full annotated script: `wgcna_after_ml.py`
How to run
Download `METABRIC_RNA_Mutation.csv` from Kaggle and `metabric_gene_modules_R.csv` from the WGCNA analysis, and place both in `data/`.
Open `wgcna_after_ml.py` in Google Colab (or a local Python/Jupyter environment).
Run top to bottom. Figures are written to `output/figures/`, tables to `output/tables/`.
Results
